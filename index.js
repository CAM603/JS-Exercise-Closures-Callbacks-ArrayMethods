...
function processLastItem(stringList, callback) {
  let lastElement = stringList[stringList.length - 1];
...
function processContains(item, list, callback) {
  let trueOrFalse = list.includes(item);
...
function processDuplicateFree(list, callback) {
  const arr = list.filter((item, index) => list.indexOf(item) === index);
  return callback(arr);
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnerSize = runners.filter(runner => runner.shirt_size === tShirtSize);
  return runnerSize;
}
